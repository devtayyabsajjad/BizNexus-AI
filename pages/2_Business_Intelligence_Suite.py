...
# Configure page settings with dark mode
st.set_page_config(
...
<style>
    ...
    .card, .resource-section { background-color: #1E1E1E; color: #E0E0E0; border: 1px solid #333; }
</style>
""", unsafe_allow_html=True)

# Initialize Groq client and load environment variables
load_dotenv()
        ...
        if metrics:
            ...
            with col1:
                ...
                <div class="metric-card">
                    ...
                    <h2>{metrics["revenue_growth"]:.2f}%</h2>
                </div>
                """, unsafe_allow_html=True)
            with col2:
                st.markdown(f"""
                <div class="metric-card">
                    <h3>Profit Margin</h3>
                    <h2>{metrics["profit_margin"]:.2f}%</h2>
                </div>
                """, unsafe_allow_html=True)
            with col3:
                st.markdown(f"""
...
def main():
    ...
    with st.sidebar:
        ...
        selected_page = st.radio(
            "Navigate to:",
            ["Business Analytics", "Document Processing", "Contract Generator", "Market Analysis"]
        )
        st.sidebar.markdown("---")
